...
async function runCommentBot(job) {
    ...
    try {
        ...
        if (!isLoggedIn) {
            ...
            // 💡 เปลี่ยนให้วิ่งเข้าหน้ามือถือตรงๆ เพื่อให้เหมือนกับที่คุณเทสในคลิป
            await page.goto('https://m.facebook.com/login', { waitUntil: 'networkidle2', timeout: 40000 });
            
            try {
                // 💡 ปรับให้หาช่องอีเมล/รหัสผ่าน ได้ทั้งรูปแบบเว็บคอมและมือถือ
                const emailSelector = 'input[name="email"], #email, #m_login_email';
                const passSelector = 'input[name="pass"], #pass, #m_login_password';
                
                await page.waitForSelector(emailSelector, { timeout: 15000 });
                
                // พิมพ์ข้อมูลและกด Enter
                await page.type(emailSelector, job.account.fbEmail, { delay: 30 });
                await page.type(passSelector, job.account.fbPassword, { delay: 30 });
                await page.keyboard.press('Enter');
                
                console.log("[FB-BOT] กดล็อกอินแล้ว รอระบบโหลด...");
                ...
                console.log("❌ [FB-BOT] หาช่องล็อกอินไม่เจอ (แคปรูปเก็บไว้แล้ว)");
                const errPath = path.join(__dirname, '../public', `error-login-${Date.now()}.png`);
